...
lm_resume <- lapply(levels(FAO_cap$zone), function(z){
  df <- makedf(z)
  
 list(#(summary(lm(tasa_FAO~year, df))),
  (summary(lm(pic.poc~year, df))))#,
  #(summary(lm(chl~year, df))),
  #(summary(lm(pic.poc~year, df))))
})
...
#VECM (lag=1)
library(tsDyn)
selec_vecm <- mdply(test_vecm[,c(1:4,18)], 
                    function(zone, variables_1, variables_2, variables_3, rank){
...
summary(cajorls_model$rlm)
normality.test(vectovar)$jb.mul$JB
...
#VECM (lag=1)
library(tsDyn)
selec_vecm <- mdply(global_test_vecm[,c(1:4,18)], function(zone, variables_1, variables_2, variables_3, rank){
